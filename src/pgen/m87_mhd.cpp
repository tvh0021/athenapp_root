...
//! \brief Problem generator for MHD galaxy cluster based on M87. By Trung Ha (UMass Amherst)
//!

/*
Similar to m87_jetclean.cpp but with a simplified large-scale weak B-field
...
namespace
{
    ...
    Real beta; // ratio of thermal energy to magnetic energy, used to set initial B-field strength
...
void Mesh::InitUserMeshData(ParameterInput *pin)
{
    ...
    restartFlag = (pin->GetOrAddInteger("job", "res_flag", 0) == 1 ? true : false);  // check whether job is new or restart
    newGridFlag = (pin->GetOrAddInteger("job", "zoom_flag", 0) == 1 ? true : false); // check whether job should use the higher resolution grid
    ...
    {
        beta = pin->GetOrAddReal("problem", "beta", 100.0); // ratio of thermal energy to magnetic energy, used to set initial B-field strength
    }
    ...
    if (Globals::my_rank == 0)
    {
        ...
        if (MAGNETIC_FIELDS_ENABLED)
        {
            std::cout << "Magnetic field is ON"
                      << "\n";
        }
        else
        {
            std::cout << "Magnetic field is OFF"
                      << "\n";
        }
        ...
        std::cout << "Number density at boundary = " << densityAtOuterBoundary / conversionNtoRho * pow(MpcCGS, -3) << " cm^-3 \n";
    ...
    ruser_mesh_data[8](0) = numberOfRefinementLevels; // only save the value at the start of the galaxy-scale simulation
        ...
        for (int j = jl; j <= ju; ++j)
        {
            ...
            for (int i = il; i <= iu; ++i)
            {
                ...
                else
                {
                    ...
                    { // Rewrite 02/09/2024: velocity perturbation is seeded randomly in x,y,z directions
                        ...
                        Real velocityPerturbationZ = sqrt(SQR(randomVelocityPerturbationAstronomical) - SQR(velocityPerturbationX) - SQR(velocityPerturbationY));

                        phydro->u(IM1, k, j, i) = interpolatedDensity / codeDensity * (velocityPerturbationX / codeVelocity);
                        phydro->u(IM2, k, j, i) = interpolatedDensity / codeDensity * (velocityPerturbationY / codeVelocity);
                        phydro->u(IM3, k, j, i) = interpolatedDensity / codeDensity * (velocityPerturbationZ / codeVelocity);
                    }
                }
            }
        }
    ...
    if (MAGNETIC_FIELDS_ENABLED)
    {
        for (int k = ks; k <= ke + 1; k++)
        {
            ...
            for (int j = js; j <= je; j++)
            {
                ...
                {
                    Real x = pcoord->x1v(i);
                    Real pressure = phydro->u(IEN, k, j, i) * gammaMinus1;
...
int RefinementCondition(MeshBlock *pmb)
{
    ...
    if (newGridFlag == true && afterGridReconstruction == false) // if the sim is at the first time step after grid reconstruction, don't refine beyond the old refinement level
    {
        requiredSMRLevel = fmin(pmb->pmy_mesh->ruser_mesh_data[8](0), int(log2(maximumRadialDistanceCode / maximumMeshBlockX)) + 1); // don't refine over the maximum number of refinement levels
    }
    else
    {
        requiredSMRLevel = fmin(requiredSMRLevel, int(log2(maximumRadialDistanceCode / maximumMeshBlockX)) + 1); // every time we go twice in radial distance, refinement level goes down by one
    }
        ...
        for (int j = pmb->js - 1; j <= pmb->je + 1; j++)
        {
            for (int i = pmb->is - 1; i <= pmb->ie + 1; i++)
            {
                if (refinementCountFlag == 0)
                {
                    ...
                    currentRefinementLevel = round(log2(rootCellLengthCode / cellLengthCode)) + 1; // when root cell length = cell length, refinement level = 1
    ...
    {
        requiredRefinementLevel = fmax(requiredAMRLevel, requiredSMRLevel); // take the maximum between the two requirements
                           ...
                           AthenaArray<Real> &cons, const AthenaArray<Real> &bcc, int k, int j, int i)
{
    ...
    if (MAGNETIC_FIELDS_ENABLED)
    {
        ...
        Real magneticEnergyDensityCode = 0.5 * (SQR(bcc1) + SQR(bcc2) + SQR(bcc3));
...
static Real simpleInterpolate(Real inputX, std::vector<Real> tabulatedX, std::vector<Real> tabulatedY)
{
    ...
    // go backwards since temperature is more likely to be on the higher end of the tabulated cooling curve
...
void enforceFloors(MeshBlock *pmb, AthenaArray<Real> &cons, int k, int j, int i)
{
    ...
    {
        cons(IDN, k, j, i) = numberDensityFloorAstronomical * conversionNtoRho / codeDensity; // hard reset of the density first; then calculate the new internal energy
